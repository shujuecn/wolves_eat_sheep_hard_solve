...
### 11.4 `./build/stat_tb` —— 表库结果分布统计

只读顺序扫描所有**已完成**桶（表头 `completed==1`，正在求解的桶自动跳过），
按 k 分别统计「狼胜 / 羊胜 / 和棋」占比，并给出"轮到狼 / 轮到羊"分项与合计。
k=4..9 全量（50.4 亿局面）约 13 秒 / 6 秒 CPU，可在求解进程运行期间随时跑，零冲突。

```bash
./build/stat_tb --data-dir data/tb_test          # 百分比表 + 合计
./build/stat_tb --data-dir data/tb_test --raw    # 附加每 k 精确原始计数
```

实现要点：每条目 1 字节（低 2 位结果），索引奇偶即回合位（奇数 = 轮到羊）；
纯顺序 mmap 读 + 位运算。注意这是**局面空间**分布（含开局不可达散点构型），
不是真实对局胜率；后者需对初始局面做可达集加权。
